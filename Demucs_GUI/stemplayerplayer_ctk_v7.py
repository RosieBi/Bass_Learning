...
#     global state
#     state = 1
    # pauseplay.configure(image=CTkImage(Image.open("pause_icon.png"), size= (30,30))
...
def separate(mp3_filename):
    ...
    listbox.insert(listbox.size(), mp3_filename[:-4])

def separate_all_songs(all_unseparated):
    for song in all_unseparated:
        separate(song)
        song_name = song[:-4]
        copy_to_new_folder(song_name)
